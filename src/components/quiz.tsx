...
export default function Quiz({ passageText, questions = mockQuiz }: QuizProps ) {
    ...
    useEffect(() => {
        async function getQuizQuestions() {
            ...
            const prompt = "Extract the following text and generate high-quality multiple choice questions to measure understanding of the text: " + passageText +
        ...
        return (
            <div className="min-h-screen flex flex-col gap-6 items-center justify-center p-10 *:max-w-md text-center">
                ...
                <div className="w-full flex flex-col gap-3">
                    <button className="w-full px-6 py-3 rounded-md bg-primary text-on disabled:text-on-disabled disabled:bg-surface-low disabled:opacity-40 transition-colors">
                        Retry at reduced speed
                    </button>
                    <button className="w-full px-6 py-3 rounded-md bg-surface-low text-on disabled:text-on-disabled disabled:bg-surface-low disabled:opacity-40 transition-colors">
                    ...
                    return (
                        <div className="flex">
                            ...
                            >
                                <div
                                    id="color-chip"
                                    className={`aspect-square w-6 h-6 flex items-center justify-center rounded-sm transition-all ${chipStyles}`}
                                >
                                    <span className={`font-bold ${chipTextStyles}`}>
                                        {String.fromCharCode(65 + idx)}
                                    </span>
                                </div>
                                <span className="py-2">{opt}</span>
                            </button>
